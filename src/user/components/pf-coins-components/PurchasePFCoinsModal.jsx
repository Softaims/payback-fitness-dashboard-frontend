...
import api from "../../../user/lib/apiClient";
...
const PurchasePFCoinsModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-white/30 flex items-center justify-center z-50 p-4">
      <div className="bg-[#0B0F0D] rounded-xl p-6 w-full max-w-lg relative">
        <button
          onClick={handleClose}
          className="cursor-pointer absolute top-4 right-4 text-[#ffffff]/50 hover:text-white transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <p className="text-[#ffffff]/50 text-sm mb-6">
          Enter the number of PF Points that you wanted to purchase.
        </p>
  ...
  [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${
    errors.coinAmount
      ...
      : "border border-[#ffffff]/10 focus:border-[#4BEEA2]"
  }`}
              />
            ...
            <button
              ...
              className={`cursor-pointer w-40 font-semibold py-3 px-4 rounded-[30px] transition-colors ${
                loading
                  ...
                  : "bg-[#4BEEA2] text-black hover:bg-[#3DD084]"
              }`}
            >
