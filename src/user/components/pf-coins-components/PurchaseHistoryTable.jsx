...
const PurchaseHistoryTable = () => {
  const { purchaseHistory, purchaseHistoryLoading, fetchPurchaseHistory } =
    usePFCoinsStore();
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#ffffff]/7">
                ...
                <th className="text-left text-[#ffffff]/50 py-3 px-4">
                  PF Points
                </th>
                <th className="text-left text-[#ffffff]/50 py-3 px-4">
                  Purchasing Date
                    ...
                    <span
                      className={`px-3 py-2 rounded-xl text-sm ${
                        item.status === "completed"
                          ? "text-[#2DD084] bg-[#2DD084]/10"
                          : "text-[#F65E53] bg-[#F65E53]/10"
                      }`}
                    >
                      {item.status}
                    </span>
            ...
            {generatePageNumbers().map((pageNum, index) => (
              <button
                ...
                className={`w-8 h-8 rounded text-sm font-medium transition-colors ${
                  pageNum === page
                    ? "bg-[#4BEEA2] text-black"
                    : pageNum === "..."
                      ? "text-[#ffffff]/50 cursor-default"
                      : "text-[#ffffff]/50 hover:text-white"
                }`}
              >
                {pageNum}
              </button>
