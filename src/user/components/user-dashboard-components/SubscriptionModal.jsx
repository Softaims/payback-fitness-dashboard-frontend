...
const SubscriptionModal = ({ isOpen, onClose }) => {
  ...
  const { plans, plansLoading, fetchSubscriptionPlans } =
    ...
    <div className="h-[100vh] min-h-screen fixed inset-0 z-[9999] bg-black/85 backdrop-blur-sm md:bg-white/30 md:flex md:items-center md:justify-center md:p-4">
      <div className="h-full w-full bg-[#0B0F0D] overflow-y-auto md:rounded-xl md:max-w-[90%] md:max-h-[95%] md:h-auto relative">
        ...
        <div className="flex md:hidden items-center justify-between p-6 bg-[#0B0F0D] sticky top-0 z-50">
          <button
            onClick={onClose}
            className="inline-flex items-center text-[#4BEEA2] hover:text-green-400 transition-colors"
          >
            ...
            <div className=" mx-auto lg:mx-0">
              ...
              <div className="mb-8">
                <h1 className="text-white text-3xl font-bold mb-2">
                  Experience <span className="text-[#4BEEA2]">Unlimited</span>{" "}
                  Features
                </h1>
                <p className="text-[#ffffff]/50 text-sm">
                  Enjoy PayBack Fitness fully with all our app has to offer
                </p>
              </div>
                  ...
                  {features.map((feature, index) => (
                    <div key={index} className="flex items-start">
                      ...
                      <div>
                        ...
                        <p className="text-[#ffffff]/50 text-xs leading-relaxed">
                          {feature.description}
                        </p>
                      </div>
                    </div>
                    ...
                    <div
                      ...
                      className={`p-4 rounded-lg border-2 cursor-pointer transition-all ${
                        selectedPlan === plan.id
                          ? "border-[#4BEEA2] bg-[#4BEEA2]/10"
                          : "border-[#FFFFFF]/7 bg-[#FFFFFF]/5 hover:border-[#FFFFFF]/20"
                      }`}
                    >
                      <div className="flex items-center justify-between">
                          ...
                          <p className="text-[#ffffff]/50 text-sm">
                            <span className="text-2xl text-[#4BEEA2]">
                              ...
                              <span className="text-2xl">
                                .{(plan.amount % 1).toFixed(2).slice(2)}
                              </span>
                            </span>{" "}
                        ...
                        >
                          {selectedPlan === plan.id && (
                            <div className="w-2 h-2 rounded-full bg-[#4BEEA2]"></div>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              <button
                ...
                className={`w-full font-semibold py-3 px-4 rounded-[30px] transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-gray-900 ${
                  loading
                    ...
                    : "bg-[#4BEEA2] cursor-pointer hover:bg-[#3dd48a]"
                }`}
              >
